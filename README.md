...
```
The parameters (Z, A etc) are in a list called dat, and the dentity-effect functions return a list of the same format. The output contains a copy of the parameters that was input to the function call plus whatever output was generated by the function. The MSP functions return a single number (i.e. the electronic mass stopping power).   
